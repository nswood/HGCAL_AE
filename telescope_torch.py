...
torch_Remap_48_36 = torch.tensor(Remap_48_36,dtype=torch.float32).to('cuda')
Weights_48_36 = SCmask_48_36[:,4]
torch_Weights_48_36 = torch.tensor(Weights_48_36,dtype=torch.float32).to('cuda')
...
torch_Remap_48_12 = torch.tensor(Remap_48_12,dtype=torch.float32).to('cuda')
Remap_12_3 = np.zeros((12,3))
for i in range(12): Remap_12_3[i,int(i/4)]=1
torch_Remap_12_3 = torch.tensor(Remap_12_3,dtype=torch.float32).to('cuda')
...
def telescopeMSE443(y_true,y_pred):
    return telescopeMSE2(torch.matmul(K.reshape(y_true,(-1,48)),remap_443_matrix),
                         torch.matmul(K.reshape(y_pred,(-1,48)),remap_443_matrix))
...
def telescopeMSE663(y_true,y_pred):
    return telescopeMSE2(torch.matmul(K.reshape(y_true,(-1,108)),remap_663_matrix),
                         torch.matmul(K.reshape(y_pred,(-1,108)),remap_663_matrix))
...
def telescopeMSE2(y_true, y_pred,device = 'cuda'):
    ...
    # lossTC1 = torch.mean(torch.square(y_true_rs - y_pred_rs), dim=(-1))
    ...
    return torch.mean(4*lossTC1 + 2*lossTC2 + lossTC3)
